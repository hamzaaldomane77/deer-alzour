...
import type { Branch, UpdateBranchRequest } from '@/types/branch';
...
export default function EditBranchPage({ params }: PageProps) {
  ...
  const fetchBranch = async () => {
    try {
      ...
      showToast({
        ...
        message: 'فشل في تحميل بيانات الفرع للتعديل',
        duration: 5000
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      await BranchService.updateBranch(parseInt(resolvedParams.id), formData);
      showToast({
        type: 'success',
      ...
      router.push(`/admin/branches/${resolvedParams.id}`);
    } catch (error) {
      showToast({
        type: 'error',
        title: 'خطأ في التعديل',
        message: 'فشل في تحديث بيانات الفرع',
        duration: 5000
      });
    } finally {
      setSaving(false);
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">تعديل الفرع</h1>
            <p className="text-green-300 text-lg">قم بتعديل بيانات الفرع</p>
          </div>
          <motion.button
            onClick={() => router.push(`/admin/branches/${resolvedParams.id}`)}
