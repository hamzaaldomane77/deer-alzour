...
import type { User, UpdatePasswordRequest } from '@/types/user';
...
export default function ChangePasswordPage({ params }: PageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.password.trim()) {
      showToast({
        type: 'error',
        title: 'يرجى إدخال كلمة المرور الجديدة',
    ...
    try {
      setLoading(true);
      await UserService.updatePassword(parseInt(resolvedParams.id), formData);
      showToast({
        type: 'success',
    ...
    <div className="max-w-2xl mx-auto">
      ...
      >
        <div className="flex items-center space-x-4 mb-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white">تغيير كلمة المرور</h1>
            <p className="text-white/70">تغيير كلمة مرور {userData.name}</p>
          </div>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="mb-8 p-6 bg-white/5 rounded-xl border border-white/10">
          <div className="flex items-center space-x-4">
            <div className="w-16 h-16 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center">
              <span className="text-white font-bold text-xl">{userData.name.charAt(0)}</span>
            </div>
            <div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4">
            <h4 className="text-yellow-300 font-medium mb-2">متطلبات كلمة المرور:</h4>
            <ul className="text-white/70 text-sm space-y-1">
              <li>• يجب أن تكون 8 أحرف على الأقل</li>
              <li>• يُنصح بخلط الأحرف والأرقام والرموز</li>
          ...
          <div className="flex justify-end space-x-4 pt-6">
            ...
            <motion.button
              type="submit"
              disabled={loading}
              className="px-6 py-3 bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              {loading ? (
