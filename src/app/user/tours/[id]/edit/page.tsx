...
export default function EditTourPage({ params }: PageProps) {
  ...
  const fetchTour = async () => {
    try {
      ...
      showToast({
        ...
        message: 'فشل في تحميل بيانات الجولة للتعديل',
        duration: 5000
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      await TourService.updateTour(parseInt(resolvedParams.id), formData);
      showToast({
        type: 'success',
      ...
      router.push(`/user/tours/${resolvedParams.id}`);
    } catch (error) {
      showToast({
        type: 'error',
        title: 'خطأ في التعديل',
        message: 'فشل في تحديث الجولة',
        duration: 5000
      });
    } finally {
      setSaving(false);
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">تعديل الجولة الميدانية</h1>
            <p className="text-green-300 text-lg">قم بتعديل بيانات جولتك الميدانية</p>
          </div>
          <motion.button
            onClick={() => router.push(`/user/tours/${resolvedParams.id}`)}
