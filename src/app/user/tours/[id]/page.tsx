...
export default function TourDetailsPage({ params }: PageProps) {
  ...
  const fetchTour = async () => {
    try {
      setLoading(true);
      const data = await TourService.getTour(parseInt(resolvedParams.id));
      setTour(data);
    } catch (error) {
      showToast({
        type: 'error',
        title: 'خطأ في تحميل الجولة',
        message: 'فشل في تحميل تفاصيل الجولة',
        duration: 5000
      });
  ...
  const handleDeleteConfirm = async () => {
    try {
      setDeleting(true);
      await TourService.deleteTour(parseInt(resolvedParams.id));
      showToast({
        type: 'success',
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: '#012623' }}>
        <div className="text-white text-center">
          ...
          <h3 className="text-xl font-bold text-white mb-2">الجولة غير موجودة</h3>
          <p className="text-green-300 mb-4">لم يتم العثور على الجولة المطلوبة</p>
          <motion.button
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">تفاصيل الجولة الميدانية</h1>
            <p className="text-green-300 text-lg">معلومات مفصلة عن جولتك الميدانية</p>
          </div>
          <div className="flex space-x-4 rtl:space-x-reverse">
            ...
            <motion.button
              onClick={() => router.push(`/user/tours/${tour.id}/edit`)}
      ...
      >
        <div className="space-y-6">
          ...
          <div className="text-center">
            ...
            <p className="text-green-300 text-lg">{tour.name}</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              <div className="flex items-center space-x-3 rtl:space-x-reverse">
                ...
                <div>
                  <h3 className="text-white font-bold">تاريخ الجولة</h3>
                  <p className="text-green-300">{formatDate(tour.date)}</p>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-center space-x-3 rtl:space-x-reverse">
                ...
                <div>
                  ...
                  <p className="text-green-300">{formatDateTime(tour.created_at)}</p>
                </div>
              </div>
            </motion.div>
          </div>
      ...
      <ConfirmDialog
        ...
        title="تأكيد الحذف"
        message={`هل أنت متأكد من حذف الجولة "${tour?.title}"؟ لا يمكن التراجع عن هذا الإجراء.`}
        confirmText="حذف"
        cancelText="إلغاء"
        isLoading={deleting}
        type="danger"
