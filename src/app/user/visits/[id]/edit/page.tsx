...
import type { Visit, UpdateVisitRequest } from '@/types/visit';
...
export default function EditVisitPage({ params }: PageProps) {
  ...
  const [formData, setFormData] = useState<UpdateVisitRequest>({
    name: '',
    title: '',
    description: '',
    date: ''
  ...
  const fetchVisit = async () => {
    try {
      ...
      showToast({
        ...
        message: 'فشل في تحميل بيانات الزيارة للتعديل',
        duration: 5000
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      await VisitService.updateVisit(parseInt(resolvedParams.id), formData);
      showToast({
        type: 'success',
      ...
      router.push(`/user/visits/${resolvedParams.id}`);
    } catch (error) {
      showToast({
        type: 'error',
        title: 'خطأ في التعديل',
        message: 'فشل في تحديث الزيارة',
        duration: 5000
      });
    } finally {
      setSaving(false);
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">تعديل الزيارة المدنية</h1>
            <p className="text-green-300 text-lg">قم بتعديل بيانات زيارتك المدنية</p>
          </div>
          <motion.button
            onClick={() => router.push(`/user/visits/${resolvedParams.id}`)}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            ...
            <input
              ...
              onChange={handleInputChange}
              className="w-full px-4 py-3 bg-white/20 border border-white/30 rounded-lg text-white placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:border-transparent transition-all duration-300"
              placeholder="أدخل عنوان الزيارة"
              required
              dir="rtl"
            />
          </motion.div>
          ...
          >
            ...
            <textarea
              ...
              placeholder="أدخل وصف مفصل للزيارة"
              required
              dir="rtl"
            />
          </motion.div>
